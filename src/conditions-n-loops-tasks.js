...
function isContainNumber(num, digit) {
  const resultStr = JSON.stringify(num);
  const resultLetter = JSON.stringify(digit);
  for (let i = 0; i < resultStr.length; i += 1) {
    if (resultStr[i] === resultLetter) {
      return true;
    }
  }
  return false;
...
function getBalanceIndex(arr) {
  ...
  for (let i = 0; i < arr.length; i += 1) {
    resultRight = result - resultLeft - arr[i];
    if (resultLeft === resultRight) return i;
    resultLeft += arr[i];
  }
  return -1;
}
